...
Статус: подготовлен, маршруты не реализованы. Договор не привязан к веб-фреймворку. Реальные сериализованные HTTP-примеры добавляются перед параллельной реализацией. expected.json авторского комплекта — эталон случаев, не тело HTTP-ответа.
...
Ошибки: 422 неверный ввод, 413 лимит размера, 404 неизвестный объект, 409 конфликт состояния, 503 недоступный сервис. Тело error: code, message, retryable. Не возвращать ключи/трассировки пользователю.
