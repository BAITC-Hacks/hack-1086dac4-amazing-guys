...
Этот файл — общая точка входа в репозиторий и ветку `main`. Подробности реализации: [backend](backend/README.md), [frontend](frontend/README.md). Backend описан по коду `b0523fd`; frontend обновлён до работы только с сервером. Материалы актуализированы **23 сентября 2026 года**.
...
Сеть нужна для первой установки зависимостей и реального анализа. Интерфейс запускается локально; получение отчёта требует работающего backend. Отдельные БД, Docker, GPU и NVIDIA-сервисы для текущей реализации не нужны. Зависимости закреплены в `backend/uv.lock` и `frontend/package-lock.json`.
...
### 2. Запустить backend для анализа
...
Для этой шкалы отдельно значимы независимый запуск и обработка неправильного ввода. Тесты ниже дают технические подтверждения; отсутствие авторизации, раннего общего лимита HTTP-тела и изоляции парсера остаётся ограничением публичного применения.
...
- **Загрузка:** по 1–5 файлов с каждой стороны, до 5 МиБ на файл; ограничения обработчика применяются после multipart-разбора. Раннего общего ограничения HTTP-тела нет.
...
- **Агент:** 250 000 символов исходных цитат, до 6 запросов модели, 8 вызовов инструментов, 600 с агентного анализа. Ожидание очереди и извлечение не включены в эти 600 с. Локальный оценочный бюджет $0.25 не является ограничением счёта у провайдера.
