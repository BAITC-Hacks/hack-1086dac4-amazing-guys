...
`runApi()` опрашивает статус с паузой 1,4 с между итерациями. Предельное ожидание после принятия задания — 10 минут; отдельный сетевой запрос имеет таймаут 30 с. Время очереди/извлечения сервера может отличаться от его 600-секундного агентного таймаута.
