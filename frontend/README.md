# Контур — руководство по frontend

«Контур» — веб-интерфейс команды Amazing Guys / HackAlem для проверки организационных изменений.
...
скопировать. В деморежиме доступен исходный Markdown-файл; для реальных документов
...
и закрывается. Во время ожидания источников появляются скелетоны, при ошибке —
объяснение и повтор загрузки. Готовые данные не задерживаются ради эффекта.
...
Данные живут в памяти вкладки; localStorage и база данных на frontend не используются.
...
│   └── demo/                  # before.md и after.md — исходники примера
├── tests/frontend.spec.ts     # Браузерные проверки
...
В наборе 22 проверки. [Отчёт о проверках и скриншоты](docs/VERIFICATION.md)
фиксирует полный прогон; после переименования кнопки отдельно прошёл основной сценарий
с JSON-выгрузкой. Автоматическая проверка доступности не заменяет ручную.
...
- Нет истории анализов между сессиями, авторизации, чата, редактирования документов
