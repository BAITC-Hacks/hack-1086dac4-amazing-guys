...
async function prepare(page: Page, evidences: Evidence[]) {
  ...
  report.function_matches[0].evidence_ids = evidences.map(
    (item) => item.evidence_id,
  );
  report.functions[0].evidence_ids = evidences
    .filter((item) => item.version === "before")
    .map((item) => item.evidence_id);
  report.functions[1].evidence_ids = evidences
  ...
  await page
    .getByRole("button", { name: "Сравнить цитаты рядом", exact: true })
    .click();
  await expect(page.locator(".change-comparison")).toBeVisible();
}

test("one-to-one card preserves Cyrillic whitespace and highlights only the added word", async ({
  page,
}) => {
  const before = "Обязанность:\n  подготовить отчёт";
  const after = "Обязанность:\n  подготовить итоговый отчёт";
  await prepare(page, [
    evidence("one-before", "before", before),
    evidence("one-after", "after", after),
  ]);
  ...
  await expect(
    card.locator(".change-comparison-context p").first(),
  ).toContainText("Проверяемый контекст");
});

test("multiple sources remain neutral and the card has no horizontal mobile overflow", async ({
  page,
}) => {
  ...
  const card = page.locator(".change-comparison");
  await expect(card.locator(".change-comparison-note")).toContainText(
...
test("oversized quotes stay exact and neutral instead of marking the whole text", async ({
  page,
}) => {
  const before = "слово ".repeat(1_700).trim();
  const after = `${before} добавлено`;
  await prepare(page, [
    evidence("long-before", "before", before),
    evidence("long-after", "after", after),
  ]);
  const card = page.locator(".change-comparison");
  await expect(card.locator(".change-comparison-note")).toContainText(
    "слишком большие",
