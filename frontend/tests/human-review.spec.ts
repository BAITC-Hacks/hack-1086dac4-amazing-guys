...
test("saves a decision and restores it after close and browser reload", async ({
  ...
  await review.locator("textarea").fill("Подтверждено по приведённому основанию");
  ...
  await page.locator(".finding-card .text-button").first().click();
  await expect(page.locator('.human-review input[value="confirmed"]')).toBeChecked();
...
test("shows a dirty draft and does not claim it was saved", async ({ page }) => {
  const review = await openFindingReview(page);
...
test("keeps the same finding id isolated between analysis keys", async ({ page }) => {
  await page.goto("/");
  await page.evaluate(() => {
    const key = [
      "kontur:human-review:v1",
      "another-analysis",
      "C2",
      "current",
    ]
